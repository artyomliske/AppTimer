...
Готовый образ для Apple Silicon доступен через официальный GitHub Release: **[AppTimer 1.4.2 — установочный DMG для arm64](https://github.com/artyomliske/AppTimer/releases/download/v1.4.2/AppTimer-1.4.2-installer-arm64.dmg)**. В образе есть ярлык `Applications`, поэтому AppTimer можно установить перетаскиванием прямо из окна DMG. Страница релиза: [AppTimer 1.4.2 — целостность данных](https://github.com/artyomliske/AppTimer/releases/tag/v1.4.2).
...
Версия **1.4.2** защищает локальный учёт от лишнего времени после аварийного закрытия AppTimer. Во время активной сессии приложение сохраняет локальную heartbeat-метку раз в 30 секунд. Если после аварийного разрыва прошло больше двух минут, незакрытый интервал автоматически закрывается последним достоверным временем, а Dashboard предлагает его проверить, отредактировать или удалить.

Сервисы уведомлений, мониторинг приложений и heartbeat теперь запускаются сразу после старта приложения, а не после первого открытия Menu Bar. Ошибки сохранения больше не игнорируются: AppTimer записывает локальную диагностику и показывает понятный статус вместо молчаливой потери данных.
